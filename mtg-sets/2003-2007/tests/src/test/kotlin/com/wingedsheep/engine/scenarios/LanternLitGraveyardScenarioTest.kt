...
import com.wingedsheep.mtg.sets.definitions.chk.cards.LanternLitGraveyard
...
/**
 * Lantern-Lit Graveyard (CHK #278):
 * {T}: Add {C}.
 * {T}: Add {B} or {R}. This land doesn't untap during your next untap step.
 */
class LanternLitGraveyardScenarioTest : FunSpec({

    val colorlessAbilityId = LanternLitGraveyard.activatedAbilities[0].id
    val firstColorAbilityId = LanternLitGraveyard.activatedAbilities[1].id
    val secondColorAbilityId = LanternLitGraveyard.activatedAbilities[2].id

    fun createDriver(): GameTestDriver {
        val driver = GameTestDriver()
        driver.registerCards(TestCards.all)
        driver.registerCard(LanternLitGraveyard)
    ...
    test("the coloured ability adds {B} and skips the controller's next untap step only") {
        val driver = createDriver()
        val player = driver.activePlayer!!
        val land = driver.putPermanentOnBattlefield(player, "Lantern-Lit Graveyard")
