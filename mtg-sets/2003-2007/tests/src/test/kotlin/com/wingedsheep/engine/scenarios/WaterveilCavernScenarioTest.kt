...
import com.wingedsheep.mtg.sets.definitions.chk.cards.WaterveilCavern
...
/**
 * Waterveil Cavern (CHK #286):
 * {T}: Add {C}.
 * {T}: Add {U} or {B}. This land doesn't untap during your next untap step.
 */
class WaterveilCavernScenarioTest : FunSpec({

    val colorlessAbilityId = WaterveilCavern.activatedAbilities[0].id
    val firstColorAbilityId = WaterveilCavern.activatedAbilities[1].id
    val secondColorAbilityId = WaterveilCavern.activatedAbilities[2].id

    fun createDriver(): GameTestDriver {
        val driver = GameTestDriver()
        driver.registerCards(TestCards.all)
        driver.registerCard(WaterveilCavern)
    ...
    test("the coloured ability adds {U} and skips the controller's next untap step only") {
        val driver = createDriver()
        val player = driver.activePlayer!!
        val land = driver.putPermanentOnBattlefield(player, "Waterveil Cavern")
