 ...
 * once each turn.
 *
 * The two trigger subjects are one union filter with OTHER binding, so Kíli never triggers for
 * itself but a permanent that is both a Dwarf and Equipment still produces only one trigger.
 */
val KiliTheResourceful = card("Kíli the Resourceful") {
    ...
    oracleText = "Storied (If you control three or more artifacts, legendaries, and/or Sagas, you " +
        ...
        "As long as you have an enduring story, you may pay {0} rather than pay the equip cost of " +
        "the first equip ability you activate each turn.\n" +
    ...
    triggeredAbility {
        trigger = Triggers.entersBattlefield(
            filter = GameObjectFilter.Creature.withSubtype(Subtype.DWARF).youControl()
                .or(GameObjectFilter.Artifact.withSubtype(Subtype.EQUIPMENT).youControl()),
