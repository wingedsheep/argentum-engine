    ...
    fun payAbilityCost(
        ...
        return when (cost) {
            ...
            is AbilityCost.Discard -> {
                ...
                val newState = result.state.recordCardsDiscardedThisTurn(discardOwner, listOf(toDiscard))
                CostPaymentResult.success(newState, manaPool, result.events)
            }
            is AbilityCost.DiscardHand -> {
                ...
                val newState = result.state.recordCardsDiscardedThisTurn(controllerId, cardsInHand)
                CostPaymentResult.success(newState, manaPool, result.events)
            }
            is AbilityCost.ExileFromGraveyard -> {
            ...
            is AbilityCost.DiscardSelf -> {
                ...
                val newState = result.state.recordCardsDiscardedThisTurn(ownerId, listOf(sourceId))
                CostPaymentResult.success(newState, manaPool, result.events)
            }
            is AbilityCost.SacrificeSelf -> {
