...
class TriggerIndexTest : FunSpec({
    ...
    context("Performance comparison") {
        test("indexed lookup is faster than unindexed for large boards") {
            val registry = TestAbilityRegistry()
            // Only one card type has triggers
            registry.register(
                "Soul Warden",
                listOf(
                    TriggeredAbility.create(
            ...
            // Create many creatures (only a few with triggers)
            val creatureCount = 100
            val wardenCount = 3 // Only 3 have triggers

            val creatureIds = mutableListOf<EntityId>()
            val wardenIds = mutableListOf<EntityId>()

            for (i in 0 until creatureCount) {
                val id = EntityId.generate()
                val definition = if (i < wardenCount) {
                    wardenIds.add(id)
                    soulWardenDef
                } else {
                    bearDef
                ...
                val (_, newState) = state.createEntity(
                    id,
                    CardComponent(definition, player1Id),
                    ControllerComponent(player1Id)
                )
                state = newState.addToZone(id, ZoneId.BATTLEFIELD)
                creatureIds.add(id)
            }
            ...
            // Warm up
            repeat(10) {
                detector.detectTriggers(state, events, registry, index)
                detector.detectTriggers(state, events, registry, null)
            }
            ...
            val indexedTime = System.nanoTime() - indexedStart

            // Time unindexed lookups
            val unindexedStart = System.nanoTime()
            repeat(1000) {
                detector.detectTriggers(state, events, registry, null)
            }
            val unindexedTime = System.nanoTime() - unindexedStart

            // Indexed should be faster
            val speedup = unindexedTime.toDouble() / indexedTime.toDouble()
            println("TriggerIndex speedup: ${speedup}x (indexed: ${indexedTime/1000000}ms, unindexed: ${unindexedTime/1000000}ms)")
            println("Index: $index")

            // Indexed should be at least somewhat faster.
            // The actual speedup depends on JIT warmup and synchronization overhead.
            // In production, the benefit is larger with more entities and fewer triggers.
            speedup shouldBeGreaterThan 1.0
        }
    }
