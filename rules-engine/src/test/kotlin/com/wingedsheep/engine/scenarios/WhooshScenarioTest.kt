 ...
 * owner's hand. If this spell was kicked, draw a card." Exercises both the unkicked bounce and the
 * kicked `WasKicked` draw branch.
 */
class WhooshScenarioTest : FunSpec({

    fun newGame(): GameTestDriver {
        val driver = GameTestDriver()
        driver.registerCards(TestCards.all + listOf(Whoosh))
    ...
    test("unkicked: returns target nonland permanent to owner's hand and does not draw") {
        ...
        val whoosh = driver.putCardInHand(you, "Whoosh!")
