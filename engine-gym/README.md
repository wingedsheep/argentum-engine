...
`:engine-gym` does that wrapping once so every downstream RL adapter
doesn't have to:
...
(which `:engine-gym-server` maps to a 400).
...
- **No HTTP/WebSocket transport.** See
  [`:engine-gym-server`](../engine-gym-server/README.md).
- **No MCTS.** See [`:engine-gym-trainer`](../engine-gym-trainer/README.md).
- **No NN inference.** The env is agnostic to featurization.
- **No training-data persistence.** `:engine-gym-trainer` handles this;
  `:engine-gym` only emits live observations.
...
./gradlew :engine-gym:test      # same via gradle
```
...
- Python trainer owns the loop → [`:engine-gym-server`](../engine-gym-server/README.md)
  (HTTP shell) + `:engine-gym` transitively.
- JVM trainer with a Python-side NN → `:engine-gym` + [`:engine-gym-trainer`](../engine-gym-trainer/README.md).
- Pure-JVM agent, no Python at all → `:engine-gym` + `:engine-gym-trainer`
  with a local-evaluator `Evaluator` implementation.
- Just want to replay a game or run one turn → `:engine-gym` alone.
