...
[ ] Cannot choose offspring (or kicker) when casting from exile or graveyard — `CastFromZoneEnumerator` never generates kicked/offspring variants, only `CastSpellEnumerator.enumerateKicker()` does (hand only)