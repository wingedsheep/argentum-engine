...
- [x] Decompose `RevealUntilEffect` — replaced with `GatherUntilMatchEffect` (walk library until filter matches, store match + all revealed) + `RevealCollectionEffect` (emit CardsRevealedEvent). Deleted `RevealUntilEffect` + `RevealUntilExecutor` (136 lines). `matchChosenCreatureType` flag replaced by composable `GameObjectFilter.Creature.withSubtypeFromVariable("chosenCreatureType")`. Updated LibraryPatterns + 2 card files (ClifftopLookout, TheInfamousCruelclaw)
- [x] Decompose `PutCreatureFromHandSharingTypeWithTappedEffect` — replaced with `EffectPatterns.putCreatureFromHandSharingTypeWithTapped()` pipeline (GatherCards(TappedAsCost) → GatherSubtypes → GatherCards(Hand, Creature.withSubtypeInEachStoredGroup) → Select → Move). Added `CardSource.TappedAsCost`, `GatherSubtypesEffect` + executor, `CardPredicate.HasSubtypeInEachStoredGroup`, `PipelineState.storedSubtypeGroups`. Deleted `PutCreatureFromHandSharingTypeWithTappedEffect` + `PutCreatureFromHandSharingTypeExecutor`
