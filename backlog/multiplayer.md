...
**Status: landed.** A new lobby **mode axis** (`LobbyGameMode.TOURNAMENT` / `FREE_FOR_ALL`),
orthogonal to the format axis, sits on `TournamentLobby` alongside `TournamentFormat` — *not* a new
format. In `FREE_FOR_ALL`, submitting the last deck (or a host start for premade) creates **one**
N-player `GameSession` seating everyone (the session layer was already N-ready after Phase 2) via a
new `FreeForAllHandler`; no `TournamentManager` is created. Wire protocol is additive: `gameMode` on
`CreateTournamentLobby`/`UpdateLobbySettings`/`LobbySettings`, plus `freeForAllGameStarting` /
`freeForAllGameComplete` / `playerEliminated` server messages (`FfaStandingInfo` placement = reverse
elimination order). In-game concede now concedes a **seat** and continues a >2 game (CR 800.4a) — the
conceder gets a personal `playerEliminated` while the table plays on; `GameSession` records the
elimination order off its state-mutation chokepoint, and `ClientPlayer.hasLost` now flags a
mid-game-eliminated player (was game-end only) so the opponent-rail tombstone renders live. Standings
come back as elimination order with a "Play Again" ready loop (same pod, new game). The lobby's
match-result/abandon/ready routes branch on `lobby.isFreeForAll`; `GamePlayHandler.handleGameOver`
now reports back for *any* lobby-linked game (was gated on a `TournamentManager`). FFA caps the pod
at 4 and rejects AI seats (AI deferred). The attacker arrow can be dragged onto the viewed opponent's
board or any rail chip to assign that defender. Client adds an `ffaState` slice + `FreeForAllOverlay`
and a lobby Mode toggle; the 2-player and tournament paths are untouched. Verified by
`FreeForAllLobbyTest` (3-player premade pod end-to-end over WebSockets: one game, mid-game concede
continues 2-way, elimination-order standings, play-again), `FfaSessionEliminationTest`, and
`MidGameEliminationVisibilityTest`.
...
- [x] 4 humans join a lobby, submit custom decks (or draft/sealed), and play one FFA game to completion
      (2–4 seats; `FreeForAllLobbyTest` covers the 3-player premade pod end-to-end)
- [x] A player conceding mid-game removes their cards and the game continues 3-way (CR 800.4a)
      (`FreeForAllLobbyTest` + `FfaSessionEliminationTest`; relies on Phase 1.2 leave-game)
- [x] Combat vs two different players in one turn works end-to-end with both defenders blocking
      (Phase 1.1 `MultiDefenderCombatTest`; client combat UX from Phase 3.6, plus the rail/board
      attacker-arrow drop target added here)
- [x] The 2-player experience (quick game, tournaments, e2e suite) is visually and behaviorally unchanged
      (all FFA chrome gated on mode/`players.length > 2`; full `:game-server` + `:rules-engine` suites green)
- [x] Spectating and replaying a 4-player game works (spectator roster from Phase 2; per-game replays
      saved + browsable via the lobby replay endpoint; FFA spectator entry via `freeForAllGameStarting`)
- [ ] One Playwright e2e: 3-player scenario game with multi-defender combat and a mid-game concede
      (deferred — the dev hotseat path, `FreeForAllLobbyTest`, and `MultiDefenderCombatTest` cover the
      same flow at the integration layer; the browser-level e2e is the lone remaining item)
