...
#### Estimates are readouts, never gates — and the numbers are the server's

The client parses `manaCostString` into pips for display and sums the server's
`availableManaSources` for a progress meter, but it no longer *derives* a cost. Every number a
payment HUD shows — the remaining cost after two cards are delved, whether the convoke selection
is payable, which lands to pre-select — comes from the server:

- **The cost preview is the one source of a partial selection's price.** The pipeline sends
  `previewCost` (see `data-contracts.md` §B1) with the draft action on entering every cost phase
  (delve, convoke, improvise/waterbend, harmonize, manual mana) and on every toggle inside one
  (`selectionSlice`'s `previewDraft` builds the draft with the same `mergeResult` the confirm path
  uses). The reply lands in `costPreview` on the pipeline slice; `CostPreviewReadout` renders it
  in the convoke / tap / harmonize bars, `DelveSelector` reads it directly, and the manual mana
  picker (`startManaSelection` + `receiveCostPreview`) prices itself off it and pre-selects the
  engine's own `autoTapPreview`. `pipelineSlice` no longer rewrites `manaCostString` between
  phases; `utils/manaCost.ts` keeps only parsing, the deckbuilder colour helpers and
  `pickConvokeColor` (which colour a creature *prefers* to pay — the server checks whether it may).
- **The caps are the server's remaining generic.** `maxDelve` and the improvise `maxTaps` start
  at every valid card / permanent and tighten to `genericRemaining + already selected` when the
  preview lands, so a player is never offered an exile the engine won't credit. (A fixed waterbend
  `{N}` and "waterbend {X}" keep their explicit cap.)
- **Harmonize is priced in the engine's order** (printed generic first, then the X) because the
  engine prices it; the client no longer has its own order to get wrong.
- **A HUD button is never disabled on a preview.** The verdict is shown beside Cast (`can't pay
  this` / the server's reason on hover), the button stays live, and the submission is what the
  server rules on. While a reply is in flight the last answer stays up, dimmed.
- **The server validates every alternative-payment choice, not just the total.**
  ...
  anything is tapped or exiled, with a message that names the permanent. The preview returns the
  same message, so the HUD says it first.
- **Blocker↔attacker pairing is the server's list.** The `DeclareBlockers` legal action carries
  `validBlockersByAttacker` (attacker → the blockers that may block it, from the same
  `BlockPhaseManager` pair rule `declareBlockers` enforces: flying, shadow, "can't be blocked by
  …", "can block only …", CR 509.1b scope) and `attackerMinBlockers` (menace = 2, "except by N or
  more"). While a blocker is dragged, only the attackers it may block keep the orange drop glow;
  `assignBlocker` refuses an unlisted pairing (`canBlockerBlock`); a menace attacker with too few
  blockers assigned wears a "+1 blocker needed" chip. Confirm stays live — the server still rules.
- **Server lists are trusted as-is.** `validCreatures` for attackers and blockers already
  ...
  `GameCard` does not re-check `isTapped` on top of it; the default attack defender is the first
...
Still client-side, deliberately: `manaCoverage.ts` (the ward / tax payment prompt's Pay gate) and
`GameBoard`'s `manaProgress` meter both match the *selected* sources against the server's cost
string — that is the player's own selection being tallied, not a rule being evaluated.
