...
That suppression only ever applies to effects generated by a **static ability** (`ContinuousEffect.fromStaticAbility`,
true for everything collected from a `ContinuousEffectSourceComponent`). A static ability generates its effect only for
as long as the object actually has that ability (CR 611.3b), so stripping the abilities stops it. A continuous effect
created by the *resolution* of a spell or ability — every floating effect — lasts as long as it says it does (CR 611.2a)
and is independent of its source's abilities: Humility never undoes a Giant Growth that already resolved, and a creature
that pumped your team with its own activated ability keeps that +1/+1 after Curious Colossus strips it.
